...
export default {
  data() {
    return {
      userInfoStyle: {
        bottom: '180px', // 将这个从 '220px' 改为 '180px'
        left: '50%',
      ...
      try {
        ...
        if (response.statusCode === 200) {
          ...
          if (result.message) {
            ...
            // 处理非最终选择的情况
            this.scenarioData = result;
            this.selectedOptionIndex = null;
            this.num = null;
            
            // 跳转到下一个页面，并传递必要的参数
            const nextPageUrl = `/pages/test/test3?jobId=${this.jobId}&userId=${this.userId}&username=${encodeURIComponent(this.username)}&gender=${this.gender}&birthday=${encodeURIComponent(JSON.stringify(this.birthday))}&options=${encodeURIComponent(JSON.stringify(this.selectedOptions))}&num=${this.num}`;
